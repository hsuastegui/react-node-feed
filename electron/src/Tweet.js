...
const Tweet = ({ tweet }) => {
  ...
  const getUrl = () => {
    if (
      tweet.hasOwnProperty("entities") &&
      tweet.entities.hasOwnProperty("urls") &&
    ...
    <article className={getClassName()}>
      <div className="row">
        <div className="col-md-12">
          <h2 className="Tweet_text">
            <a href={getUrl()} target="_blank">{cleanText()}</a>
          </h2>
        </div>
        <div className="col-md-6">
          <small className="Tweet_date">
            {moment(tweet.created_at, "ddd MMM DD HH:mm:ss Z YYYY").format(
              "LLLL"
            )}
          </small>
        </div>
